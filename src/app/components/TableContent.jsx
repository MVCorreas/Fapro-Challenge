...
export const TableContent = () => {
  ...
  const fetchEntities = async () => {
    ...
    try {
      ...
      const enabledEntities = response.data.data.filter(
        (entity) => entity.is_enabled
      );
      const disabledEntitiesList = response.data.data.filter(
        (entity) => !entity.is_enabled
      );
...
//fetchEntities fx logic outside the useEffects make it reusable elsewhere in the component (e.g. removeEntity fx)
  ...
  const updateEntityStatus = async (id, newStatus) => {
    ...
    try {
      await axios.patch(
        `${apiUrl}/api_entities/entities/${id}/`,
    ...
    if (window.confirm("Do you want to remove?")) {
      ...
      try {
        ...
        if (response.status === 204) {
          ...
          setDisabledEntities((prevDisabledEntities) =>
            prevDisabledEntities.filter((entity) => entity.id !== id)
          );
        } else {
          console.error(
            `Failed to delete entity, status code: ${response.status}`
          );
        }
  ...
  const handleSelectAll = (e) => {
    if (e.target.checked) {
      const allEntitiesIds = [
        ...
        ...disabledEntities.map((entity) => entity.id),
      ];
      ...
      entitiesList.map((entity) => (
        <tr key={entity.id}>
          ...
          <td className="text-xs">
            {new Date(entity.created_at).getFullYear()}
          </td>
          <td>
                ...
                <label>
                  <input
                    ...
                    checked={
                      ...
                      [...entities, ...disabledEntities].length
                    }
                  />
                </label>
              </th>
